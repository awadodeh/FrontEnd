...
public class EditSalesOrder extends EditContentPanel
{
	...
	public boolean bindToGUI(Object o) {
		...
		txtOrderNum.setText(salesOrder.getOrderNumber()+"");
		txtTotalPrice.setText(salesOrder.getTotalPrice()+"");
		
		for (OrderLine orderLine : salesOrder.getOrderLines()) {
			defaultTableModel.addRow(new String[] { orderLine.getProduct().getCode()+"", "" + orderLine.getQuantity(), "" + orderLine.getProduct().getPrice(), "" + (orderLine.getQuantity() * orderLine.getProduct().getPrice()) });
		}
		return true;
	}
	...
	public Object guiToObject() {
		...
		for (int rowIndex = 0; rowIndex < defaultTableModel.getRowCount(); rowIndex++) {
			...
			orderLine.setProduct((Product)Services.readRecordByCode((String)defaultTableModel.getValueAt(rowIndex, 0), Services.TYPE_PRODUCT));
			orderLine.setQuantity(Long.parseLong((String)defaultTableModel.getValueAt(rowIndex, 1)));
			orderLines.add(orderLine);
		}
		
		salesOrder.setOrderLines(orderLines);
		salesOrder.setCustomer((Customer)Services.readRecordByCode(((ComboBoxItem) txtCustomer.getSelectedItem()).getKey(), Services.TYPE_CUSTOMER));
